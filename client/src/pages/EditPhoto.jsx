...
const EditPhoto = () => {
  ...
  const applyAllEffects = useCallback(
    (img) => {
      ...
      if (filters.blackWhite)
        applyBlackWhiteFilter(ctx, canvas.width, canvas.height);
      ...
      if (filters.valencia)
        applyValenciaFilter(ctx, canvas.width, canvas.height);
      if (filters.moon) applyMoonFilter(ctx, canvas.width, canvas.height);
      if (filters.robeautify)
        applyRobeautifyFilter(ctx, canvas.width, canvas.height);
  ...
  const applyGothamFilter = (ctx, width, height) => {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      data[i] = 128 + (data[i] - 128) * contrastFactor; // R
      data[i + 1] = 128 + (data[i + 1] - 128) * contrastFactor; // G
      data[i + 2] = 128 + (data[i + 2] - 128) * contrastFactor; // B

      data[i] *= 0.9;
      data[i + 1] *= 0.85;
      data[i + 2] = Math.min(220, data[i + 2] * 1.1); // Boost blue slightly
  ...
  const applyLofiFilter = (ctx, width, height) => {
    ...
    for (let i = 0; i < data.length; i += 4) {
      // Apply low contrast and warm tones
      const r = data[i],
        g = data[i + 1],
        b = data[i + 2];
        ...
        if (Math.random() < decorDensity) {
          ...
          if (brightness > 170) {
            ...
            if (Math.random() > 0.3) {
              ...
              decorCtx.fillStyle = `rgba(255, 220, 230, ${
                0.5 + Math.random() * 0.3
              })`;
              ...
              // 70% sparkles
              const size = sizes[Math.floor(Math.random() * sizes.length)];
              decorCtx.fillStyle = `rgba(255, 255, 255, ${
                0.6 + Math.random() * 0.3
              })`;
    ...
    if (edits.saturation !== 0 || edits.temperature !== 0) {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        if (edits.saturation !== 0) {
          ...
          data[i + 1] =
            gray + (data[i + 1] - gray) * (1 + edits.saturation / 100);
          data[i + 2] =
            gray + (data[i + 2] - gray) * (1 + edits.saturation / 100);
        }
  ...
  const getNearHandle = (x, y, rect) => {
    ...
    for (const handle of handles) {
      if (
        Math.sqrt(Math.pow(x - handle.x, 2) + Math.pow(y - handle.y, 2)) < 10
      ) {
        return handle.type;
      }
    }
  ...
  const resizeCropRect = (rect, handle, mouseX, mouseY, aspectRatio) => {
    ...
    let newRect = { ...rect };
    const ratio =
      aspectRatio === "custom" ? null : getAspectRatio(aspectRatio, image);
  ...
  const getHoverHandle = (x, y) => {
    ...
    for (const handle of handles) {
      if (
        Math.sqrt(Math.pow(x - handle.x, 2) + Math.pow(y - handle.y, 2)) <
        handleSize
      ) {
        return handle.type;
      }
  ...
  const rotateImage = async (direction) => {
    ...
    rotatedImg.onload = () => {
      ...
      // Update state with new image and rotation
      setImage(rotatedImg);
  ...
  const applyCrop = () => {
    ...
    croppedImg.onload = () => {
      ...
      // Set canvas size to cropped image dimensions
      canvas.width = width;
      canvas.height = height;
      const ctx = canvas.getContext("2d");
      ctx.drawImage(croppedImg, 0, 0);

      // Save cropped image to sessionStorage
      const dataUrl = canvas.toDataURL("image/jpeg");
  ...
  const handleToolChange = (toolName, value) => {
    ...
    if (toolName === "crop" && value && image) {
      ...
      // Adjust crop rectangle to new aspect ratio
    ...
    try {
      ...
      if ((action === "save" || action === "saveAndDownload") && currentUser) {
        ...
        const saveResponse = await fetch(
          "http://localhost:5000/api/save-photo",
          {
            method: "POST",
            headers: {
              ...
              <canvas
                ...
                style={{
                  ...
                  cursor: tools.crop
                    ? isCropping
                      ? "grabbing"
                      : getHoverHandle(mouseX, mouseY)
                      ? `${getHoverHandle(mouseX, mouseY)}-resize`
                      : "move"
                    : "default",
                }}
              />
            ...
            <Toolbar>
              <LeftGroup>
                ...
                <ToolButton
                  ...
                  onClick={() =>
                    setActiveTab(activeTab === "edit" ? null : "edit")
                  }
                >
                  <FaSlidersH /> Edit
                </ToolButton>
              ...
              <SubMenu>
                {activeTab === "filters" && (
                  <FilterOptions>
                    ...
                    <FilterOption
                      $active={filters.hudson}
                      onClick={() => toggleFilter("hudson")}
                    >
                      <FaMountain /> Hudson
                    </FilterOption>
                    <FilterOption
                      $active={filters.amaro}
                      onClick={() => toggleFilter("amaro")}
                    >
                      <FaFire /> Amaro
                    </FilterOption>
                    <FilterOption
                      $active={filters.xpro}
                      onClick={() => toggleFilter("xpro")}
                    >
                      <FaBolt /> X-Pro
                    </FilterOption>
                    <FilterOption
                      $active={filters.sierra}
                      onClick={() => toggleFilter("sierra")}
                    >
                      <FaUmbrellaBeach /> Sierra
                    </FilterOption>
                    <FilterOption
                      $active={filters.valencia}
                      onClick={() => toggleFilter("valencia")}
                    >
                      <FaPaintBrush /> Valencia
                    </FilterOption>
                    <FilterOption
                      $active={filters.moon}
                      onClick={() => toggleFilter("moon")}
                    >
                      <FaRegMoon /> Moon
                    </FilterOption>
                    <FilterOption
                      $active={filters.robeautify}
                      onClick={() => toggleFilter("robeautify")}
                    >
                      <FaHeart /> Robeautify
                    </FilterOption>
                  </FilterOptions>
                    ...
                    <ToolOption>
                      <label>
                        <input
                          ...
                          checked={tools.crop}
                          onChange={(e) =>
                            handleToolChange("crop", e.target.checked)
                          }
                        />
                        Enable Crop
                      </label>
                      ...
                      <>
                        <ToolOption>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              handleToolChange("aspectRatio", e.target.value)
                            }
                          >
                  ...
                  <EditOptions>
                    <EditSlider>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleEditChange("exposure", e.target.value)
                        }
                        step="1"
                      />
                    </EditSlider>
                    <EditSlider>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleEditChange("contrast", e.target.value)
                        }
                        step="1"
                      />
                    </EditSlider>
                    <EditSlider>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleEditChange("saturation", e.target.value)
                        }
                        step="1"
                      />
                    </EditSlider>
                    <EditSlider>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleEditChange("temperature", e.target.value)
                        }
                        step="1"
                      />
                    </EditSlider>
...
const CanvasContainer = styled.div`
  ...
  background-color: ${(props) =>
    props.$dragActive
      ...
      : "#f5f5f5"};
  border: ${(props) =>
    props.$dragActive ? "2px dashed #4a90e2" : "1px dashed #ddd"};
  border-radius: 8px;
  transition: all 0.2s;
  position: relative;
  overflow: hidden;
